...
class MLPFunction(Parameterized, Serializable):
	###
    def __init__(self, name, input_pls, hidden_layer_sizes, outputndims,
	###
    ...
    def __call__(self, states):
        ...
        embedded = self._mlpfunction_net(inputs = states,
                                         output_nonlinearity = self._output_nonlinearity,
                                         layer_sizes=self._layer_sizes)
        print("embedded_1 : ", embedded)
        if self._ndims is not None:
            embedded = embedded[..., :self._ndims]
        if not batched:
            print("if not batched: : ", embedded[0])
            return embedded[0]
        print("embedded_2 : ", embedded)
        return embedded
	###
