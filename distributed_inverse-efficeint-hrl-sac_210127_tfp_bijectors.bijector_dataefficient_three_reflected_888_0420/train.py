...
FLAGS = flags.FLAGS
###
flags.DEFINE_string('goal_sample_strategy', 'RealNVP',
                    'None, sample, FuN, RealNVP')
###
LOAD_PATH = None
...
def collect_experience(tf_env, agent, meta_agent, state_preprocess,
  ...
  ###
  action_vae, action_z_state, action_real_nvp = action_fn(state, context=None)
  action = action_vae
  ###
  ...
  with tf.control_dependencies(collect_experience_ops):
    ...
    with tf.variable_scope(tf.get_variable_scope(), reuse=tf.AUTO_REUSE):
      ...
      #####									
      actions_var_z_state = tf.get_variable('actions_var_z_state',
                                    [meta_action_every_n, action_z_state.shape[-1]],
                                    action_z_state.dtype)
      actions_var_real_nvp = tf.get_variable('actions_var_real_nvp',
                                    [meta_action_every_n, action_real_nvp.shape[-1]],
                                    action_real_nvp.dtype)
      #####									
    ...
    ###
    actions_var_upd_z_state = tf.scatter_update(
        actions_var_z_state, (agent.tf_context.t - 2) % meta_action_every_n, action_z_state)
    actions_var_upd_real_nvp = tf.scatter_update(
        actions_var_real_nvp, (agent.tf_context.t - 2) % meta_action_every_n, action_real_nvp)
    ###
    ###
    with tf.control_dependencies([actions_var_upd, actions_var_upd_z_state, actions_var_upd_real_nvp]):
    ###
      actions = tf.identity(actions_var) + tf.zeros_like(actions_var)
      ###	  
      actions_z_state = tf.identity(actions_var_z_state) + tf.zeros_like(actions_var_z_state)	  
      actions_real_nvp = tf.identity(actions_var_real_nvp) + tf.zeros_like(actions_var_real_nvp)
      ###	  
    ...
    meta_transition = [state_var, meta_action_var,
                       ...
                       next_state]
    ###
    meta_transition.extend([states_var, actions, actions_z_state, actions_real_nvp])
    if store_context:  # store current and next context into replay
	  ###
      next_context = list(agent.context_vars)
      next_action_vae, next_action_z_state, next_action_real_nvp = action_fn(next_state, next_context)
      transition.extend([action_z_state, action_real_nvp, next_action_vae, next_action_z_state, next_action_real_nvp])
      ###
      transition += context + next_context
...
def train_uvf(train_dir,
  ...
  ###
  meta_action_fn = meta_agent.meta_action
  meta_action_fn = meta_agent.add_noise_fn(meta_action_fn, global_step=None)

  meta_actions_fn = meta_agent.meta_actions
  ...
  for mode in ['meta', 'nometa']:
    ...
    with tf.name_scope(mode):
      ...
      if mode == 'meta':
        ...
        #####
        low_actions_z_state = batch_dequeue[7]
        low_actions_real_nvp = batch_dequeue[8]
        #####
        low_state_reprs = state_preprocess(low_states)
	  ###		
      else:
        nometa_action_z_states = batch_dequeue[5]
        nometa_action_real_nvps = batch_dequeue[6]
        nometa_next_action_vaes = batch_dequeue[7]
        nometa_next_action_z_states = batch_dequeue[8]
        nometa_next_action_real_nvps = batch_dequeue[9]
	  ###
      ...
      if mode == 'meta':  # Re-label meta-action
        ...
        if FLAGS.goal_sample_strategy == 'None':
          ...
          actions = sample_best_meta_actions(state_reprs, next_state_reprs, prev_actions,
                                             ...
                                             inverse_dynamics, uvf_agent, k=10)
		###
        elif FLAGS.goal_sample_strategy == 'RealNVP':
          actions_rnvp = uvf_agent.inverse_real_nvp(low_actions_real_nvp, low_actions_z_state)
          actions = actions_rnvp
		###
        else:
          assert False
	  ...
	  ###
      if mode == 'nometa':
        merged_states_actions = agent.merged_states(states, [actions])
        merged_next_states_actions = agent.merged_states(next_states, [nometa_next_action_vaes])
        merged_context_z_states = agent.merged_states(nometa_action_z_states, contexts)
        merged_next_context_z_states = agent.merged_states(nometa_next_action_z_states, next_contexts)
	  ###
	  ###
      if mode == 'meta':
	  ###
	  ...
	  ###
        ...
        #### be careful for this order : action_z -> action -> action_real_nvp
        critic_loss_vae = agent.critic_loss_vae(merged_states, actions,
                                      context_rewards, context_discounts,
                                      merged_next_states)

        critic_loss_z_state = agent.critic_loss(merged_states_actions, nometa_action_z_states,
                                      context_rewards, context_discounts,
                                      merged_next_states_actions)

        critic_loss_real_nvp = agent.critic_loss_real_nvp(merged_context_z_states, nometa_action_real_nvps,
                                      context_rewards, context_discounts,
                                      merged_next_context_z_states, original_states = nometa_action_z_states, original_next_states = nometa_next_action_z_states, next_contexts = next_contexts)

        critic_loss = ( critic_loss_vae + critic_loss_z_state + critic_loss_real_nvp ) / 3
	  ...
	  ###
        ...
        actor_loss_vae = agent.actor_loss_vae(merged_states, actions,
                                    context_rewards, context_discounts,
                                    merged_next_states)

        actor_loss_z_state = agent.actor_loss(merged_states_actions, nometa_action_z_states,
                                    context_rewards, context_discounts,
                                    merged_next_states_actions)
        actor_loss_real_nvp = agent.actor_loss_real_nvp(merged_context_z_states, nometa_action_z_states, contexts)
        actor_loss = (actor_loss_vae + actor_loss_z_state + actor_loss_real_nvp) / 3
	  ###
      ...
      if mode == 'nometa':
        ...
        train_op_list.append(critic_train_op)
          ...
          critic_train_meta_op = slim.learning.create_train_op(
              critic_loss,
              critic_opt,
              global_step=n_updates,
              update_ops=None,
              summarize_gradients=summarize_gradients,
              clip_gradient_norm=clip_gradient_norm,
              variables_to_train=agent.get_trainable_critic_meta_vars(), )
          critic_train_meta_op = uvf_utils.tf_print(
              critic_train_meta_op, [critic_train_meta_op],
              message='critic_loss_meta',
              print_freq=1000,
              name='critic_loss_meta')
          train_op_list.append(critic_train_meta_op)
	  ###
      if actor_loss is not None:
        ...
        if mode == 'nometa':
            ...
            actor_train_meta_op = slim.learning.create_train_op(
                actor_loss,
                actor_opt,
                global_step=None,
                update_ops=None,
                summarize_gradients=summarize_gradients,
                clip_gradient_norm=clip_gradient_norm,
                variables_to_train=agent.get_trainable_actor_meta_vars(), )
            actor_train_meta_op = uvf_utils.tf_print(
                actor_train_meta_op, [actor_train_meta_op],
                message='actor_loss_meta',
                print_freq=1000,
                name='actor_loss_meta')
            train_op_list.append(actor_train_meta_op)
		###
  ...
  if meta_agent is not None:
    with tf.control_dependencies(train_op_list[:2]):
      ...
      update_meta_targets_op = uvf_utils.periodically(
	  	  meta_agent.update_targets_meta, target_update_period, 'update_targets_meta')
	  ###
  ...
  ###
  policy_vars = uvf_agent.get_actor_vars() + meta_agent.get_actor_meta_vars() + [
  ...
  ###
  policy_vars += uvf_agent.get_trainable_critic_vars() + meta_agent.get_trainable_critic_meta_vars()
  ###
