...
def critic_net(states, actions,
  ...
  return value
###
@gin.configurable('ddpg_critic_net_vae')
def critic_net_vae(states, actions,
               for_critic_loss=False,
               num_reward_dims=1,
               states_hidden_layers=(400,),
               actions_hidden_layers=None,
...
@gin.configurable('ddpg_critic_net_real_nvp')
def critic_net_real_nvp(states, actions,
               for_critic_loss=False,
               num_reward_dims=1,
               states_hidden_layers=(400,),
               actions_hidden_layers=None,
...
###
###
@gin.configurable('ddpg_actor_net_meta')
def actor_net_meta(states, action_spec,
...
def actor_net(states, action_spec,
  ...
  return actions
###
@gin.configurable('ddpg_actor_net_vae')
def actor_net_vae(states, action_spec,
              hidden_layers=(400, 300),
              normalizer_fn=None,
              activation_fn=tf.nn.relu,
              zero_obs=False,
...
@gin.configurable('ddpg_real_nvp_actor_net')
def real_nvp_actor_net(self, states, contexts=None):

  states = tf.to_float(states)
  contexts = tf.to_float(contexts)
  actions = self._bijector.forward(
          contexts, condition = states)

  actions = tf.tanh(actions) if self._squash else actions

  action_means = (self._action_spec.maximum + self._action_spec.minimum) / 2.0
  action_magnitudes = (self._action_spec.maximum - self._action_spec.minimum) / 2.0
  actions = action_means + action_magnitudes * actions

  return actions


def target_real_nvp_actor_net(self, states, contexts=None):
  states = tf.to_float(states)
  contexts = tf.to_float(contexts)

  actions = self._target_bijector.forward(
          contexts, condition = states)
