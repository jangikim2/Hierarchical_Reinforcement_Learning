...
class DdpgAgent(object):
  ...
  """
  ###
  ACTOR_NET_META_SCOPE = 'actor_net_meta'
  CRITIC_NET_META_SCOPE = 'critic_net_meta'
  TARGET_ACTOR_NET_META_SCOPE = 'target_actor_net_meta'
  TARGET_CRITIC_NET_META_SCOPE = 'target_critic_net_meta'
  ###
  ...
  ### vae
  ACTOR_NET_VAE_SCOPE = 'actor_net_vae'
  CRITIC_NET_VAE_SCOPE = 'critic_net_vae'
  TARGET_ACTOR_NET_VAE_SCOPE = 'target_actor_net_vae'
  TARGET_CRITIC_NET_VAE_SCOPE = 'target_critic_net_vae'

  ### real_nvp
  ACTOR_NET_REAL_NVP_SCOPE = 'actor_net_real_nvp'
  CRITIC_NET_REAL_NVP_SCOPE = 'critic_net_real_nvp'
  TARGET_ACTOR_NET_REAL_NVP_SCOPE = 'target_actor_net_real_nvp'
  TARGET_CRITIC_NET_REAL_NVP_SCOPE = 'target_critic_net_real_nvp'
  ...
  def __init__(self,
               ...
               observation_spec,
			   ###
               original_observation_spec,
               original_context_specs,
			   ###
               action_spec,
			   ###
               num_state_dims,
               actor_net_meta=networks.actor_net_meta,
			   ###
               actor_net=networks.actor_net,
			   ###
               actor_net_vae=networks.actor_net_vae,
               actor_net_real_nvp=networks.real_nvp_actor_net,
               target_actor_net_real_nvp=networks.target_real_nvp_actor_net,
			   ###
               critic_net=networks.critic_net,
			   ###
               critic_net_vae=networks.critic_net_vae,
               critic_net_real_nvp=networks.critic_net_real_nvp,
			   ###
    ...
    if self.mode == 'uvf':
        ...
        ### vae
        self._actor_net_vae = tf.make_template(
            self.ACTOR_NET_SCOPE, actor_net_vae, create_scope_now_=True)
        self._critic_net_vae = tf.make_template(
            self.CRITIC_NET_SCOPE, critic_net_vae, create_scope_now_=True)
        self._target_actor_net_vae = tf.make_template(
            self.TARGET_ACTOR_NET_SCOPE, actor_net_vae, create_scope_now_=True)
        self._target_critic_net_vae = tf.make_template(
            self.TARGET_CRITIC_NET_SCOPE, critic_net_vae, create_scope_now_=True)

        ### real_nvp
        self._actor_net_real_nvp = tf.make_template(
            self.ACTOR_NET_SCOPE, actor_net_real_nvp, create_scope_now_=True)
        self._critic_net_real_nvp = tf.make_template(
            self.CRITIC_NET_SCOPE, critic_net_real_nvp, create_scope_now_=True)
        self._target_actor_net_real_nvp = tf.make_template(
            self.TARGET_ACTOR_NET_SCOPE, target_actor_net_real_nvp, create_scope_now_=True)
        self._target_critic_net_real_nvp = tf.make_template(
            self.TARGET_CRITIC_NET_SCOPE, critic_net_real_nvp, create_scope_now_=True)
    else:
        self._actor_net_meta = tf.make_template(
            self.ACTOR_NET_SCOPE, actor_net_meta, create_scope_now_=True)
        self._critic_net = tf.make_template(
            self.CRITIC_NET_SCOPE, critic_net, create_scope_now_=True)
        self._target_actor_net_meta = tf.make_template(
            self.TARGET_ACTOR_NET_SCOPE, actor_net_meta, create_scope_now_=True)
        self._target_critic_net = tf.make_template(
            self.TARGET_CRITIC_NET_SCOPE, critic_net, create_scope_now_=True)
	###
  ...
  def actions(self, state):
    ...
    Returns:
      A [num_action_dims] tensor representing the action.
    """
    return self.actor_net(self._batch_state(state), stop_gradients=True)

  def action_vae(self, state):
    """Returns the next action for the state.
    ...
    Returns:
      A [num_action_dims] tensor representing the action.
    """
    return self.actor_net_vae(self._batch_state(state), stop_gradients=True)[0, :]

  def action_vaes(self, state):
    """Returns the next action for the state.
    ...
    Returns:
      A [num_action_dims] tensor representing the action.
    """
    return self.actor_net_vae(self._batch_state(state), stop_gradients=True)

  def action_real_nvp(self, state, context):
    """Returns the next action for the state.
    ...
    Returns:
      A [num_action_dims] tensor representing the action.
    """
    return self.actor_net_real_nvp(self._batch_state(state), self._batch_state(context), stop_gradients=True)[0, :]

  def action_real_nvps(self, state, context):
    """Returns the next action for the state.
    ...
    Returns:
      A [num_action_dims] tensor representing the action.
    """
    return self.actor_net_real_nvp(self._batch_state(state), self._batch_state(context), stop_gradients=True)
  ###
  ...
  def actor_net(self, states, stop_gradients=False):
	...
	###
        ...
        actions = self._actor_net_meta(states, self._action_spec)
	###
    if stop_gradients:
      actions = tf.stop_gradient(actions)
    return actions
  ###
  def actor_net_vae(self, states, stop_gradients=False):
    """Returns the output of the actor network.
    ...
    """
    self._validate_states(states)
    actions = self._actor_net_vae(states, self._action_spec)
    if stop_gradients:
      actions = tf.stop_gradient(actions)
    return actions

  def actor_net_real_nvp(self, states, contexts, stop_gradients=False):
    """Returns the output of the actor network.
    ...
    Raises:
      ValueError: If `states` does not have the expected dimensions.
    """
    self._validate_original_states_contexts(states, contexts)

    if type(contexts) == list:
        contexts = contexts[0]

    actions = self._actor_net_real_nvp(self, states, contexts)
  ...
  ###
  def critic_net_vae(self, states, actions, for_critic_loss=False):
    """Returns the output of the critic network.
    ...
    self._validate_states(states)
    self._validate_actions(actions)
    return self._critic_net_vae(states, actions,
                            for_critic_loss=for_critic_loss)

  def critic_net_real_nvp(self, states, actions, for_critic_loss=False):
    """Returns the output of the critic network.
    ...
    self._validate_states(states)
    self._validate_actions(actions)
    return self._critic_net_real_nvp(states, actions,
                            for_critic_loss=for_critic_loss)
  ###
  def target_actor_net(self, states):
	...
	###
    if self.mode == 'uvf':
        print("target_actor_net_uvf_states : ", states)
	###
        actions = self._target_actor_net(states, self._action_spec)
	###
    else:
        print("target_actor_net_meta_states : ", states)
        actions = self._target_actor_net_meta(states, self._action_spec)
	###
    return tf.stop_gradient(actions)
  ###
  def target_actor_net_vae(self, states):
    """Returns the output of the target actor network.
    ...
    """
    self._validate_states(states)
    actions = self._target_actor_net_vae(states, self._action_spec)
    return tf.stop_gradient(actions)


  def target_actor_net_real_nvp(self, states, contexts):
    """Returns the output of the target actor network.
    ...
    Raises:
      ValueError: If `states` does not have the expected dimensions.
    """
    self._validate_original_states_contexts(states, contexts[0])
    actions = self._target_actor_net_real_nvp(self, states, contexts[0])
  ...
  def target_critic_net(self, states, actions, for_critic_loss=False):
    ...
    return tf.stop_gradient(
        self._target_critic_net(states, actions,
                                for_critic_loss=for_critic_loss))
  ###
  def target_critic_net_vae(self, states, actions, for_critic_loss=False):
    """Returns the output of the target critic network.
    ...
    self._validate_actions(actions)
    return tf.stop_gradient(
        self._target_critic_net_vae(states, actions,
                                for_critic_loss=for_critic_loss))

  def target_critic_net_real_nvp(self, states, actions, for_critic_loss=False):
    """Returns the output of the target critic network.
    ...
    self._validate_actions(actions)
    return tf.stop_gradient(
        self._target_critic_net_real_nvp(states, actions,
                                for_critic_loss=for_critic_loss))
  ###
  def value_net(self, states, for_critic_loss=False):
    ...
    return self.critic_net(states, actions,
                           for_critic_loss=for_critic_loss)
  ###
  def value_net_vae(self, states, for_critic_loss=False):
    """Returns the output of the critic evaluated with the actor.
    ...
    Returns:
      q values: A [batch_size] tensor of q values.
    """
    actions = self.actor_net_vae(states)
    return self.critic_net_vae(states, actions,
                           for_critic_loss=for_critic_loss)

  def value_net_real_nvp(self, states, original_states, contexts, for_critic_loss=False):
    """Returns the output of the critic evaluated with the actor.
    ...
    Returns:
      q values: A [batch_size] tensor of q values.
    """
    actions = self.actor_net_real_nvp(original_states, contexts)
    return self.critic_net_real_nvp(states, actions,
                           for_critic_loss=for_critic_loss)
  ###
  def target_value_net(self, states, for_critic_loss=False):
    ...
    return self.target_critic_net(states, target_actions,
                                  for_critic_loss=for_critic_loss)
  ###
  def target_value_net_vae(self, states, for_critic_loss=False):
    """Returns the output of the target critic evaluated with the target actor.
    ...
    Returns:
      q values: A [batch_size] tensor of q values.
    """
    target_actions = self.target_actor_net_vae(states)
    return self.target_critic_net_vae(states, target_actions,
                                  for_critic_loss=for_critic_loss)

  def target_value_net_real_nvp(self, states, original_states, contexts, for_critic_loss=False):
    """Returns the output of the target critic evaluated with the target actor.
    ...
    Returns:
      q values: A [batch_size] tensor of q values.
    """
    target_actions = self.target_actor_net_real_nvp(original_states, contexts)
    return self.target_critic_net_real_nvp(states, target_actions,
                                  for_critic_loss=for_critic_loss)
  ###
  def critic_loss(self, states, actions, rewards, discounts,
    ...
    return loss
  ###
  def critic_loss_vae(self, states, actions, rewards, discounts,
                  next_states):
    """Computes a loss for training the critic network.
    ...
    target_q_values = self.target_value_net_vae(next_states, for_critic_loss=True)
    td_targets = target_q_values * discounts + rewards
    if self._target_q_clipping is not None:
      td_targets = tf.clip_by_value(td_targets, self._target_q_clipping[0],
                                    self._target_q_clipping[1])
    q_values = self.critic_net_vae(states, actions, for_critic_loss=True)
    td_errors = td_targets - q_values
    if self._debug_summaries:
      gen_debug_td_error_summaries(
          target_q_values, q_values, td_targets, td_errors)
    ...
    if self._residual_phi > 0.0:  # compute residual gradient loss
      residual_q_values = self.value_net_vae(next_states, for_critic_loss=True)
      residual_td_targets = residual_q_values * discounts + rewards
      if self._target_q_clipping is not None:
        residual_td_targets = tf.clip_by_value(residual_td_targets,
                                               self._target_q_clipping[0],
  ...
  def critic_loss_real_nvp(self, states, actions, rewards, discounts,
                      next_states, original_states, original_next_states, next_contexts):
    """Computes a loss for training the critic network.
    ...
    target_q_values = self.target_value_net_real_nvp(next_states, original_next_states, next_contexts, for_critic_loss=True)
    td_targets = target_q_values * discounts + rewards
    if self._target_q_clipping is not None:
      td_targets = tf.clip_by_value(td_targets, self._target_q_clipping[0],
                                    self._target_q_clipping[1])
    q_values = self.critic_net_real_nvp(states, actions, for_critic_loss=True)
    td_errors = td_targets - q_values
    if self._debug_summaries:
      gen_debug_td_error_summaries(
          target_q_values, q_values, td_targets, td_errors)
    ...
    if self._residual_phi > 0.0:  # compute residual gradient loss
      residual_q_values = self.value_net_real_nvp(next_states, original_next_states, next_contexts, for_critic_loss=True)
      residual_td_targets = residual_q_values * discounts + rewards
      if self._target_q_clipping is not None:
        residual_td_targets = tf.clip_by_value(residual_td_targets,
                                               self._target_q_clipping[0],
  ...
  ###
  def actor_loss_vae(self, states):
    """Computes a loss for training the actor network.
    ...
    """
    self._validate_states(states)
    actions = self.actor_net_vae(states, stop_gradients=False)
    critic_values = self.critic_net_vae(states, actions)
    q_values = self.critic_function(critic_values, states)
    dqda = tf.gradients([q_values], [actions])[0]
    dqda_unclipped = dqda
    if self._dqda_clipping > 0:
  ...
  def actor_loss_real_nvp(self, states, original_states, contexts):
    """Computes a loss for training the actor network.
    ...
    """
    self._validate_states(states)
    print("actor_loss_real_nvp_original_states : ", original_states)
    print("actor_loss_real_nvp_original_contexts : ", contexts)
    actions = self.actor_net_real_nvp(original_states, contexts, stop_gradients=False)
    critic_values = self.critic_net_real_nvp(states, actions)
  ...
  ###
  @gin.configurable('ddpg_update_targets_meta')
  def update_targets_meta(self, tau=1.0):
    ...
    update_critic = utils.soft_variables_update(
        ...
        tau)
    return tf.group(update_actor, update_critic, name='update_targets_meta')
  ###
  ...
  ###
  @gin.configurable('ddpg_update_targets_vae')
  def update_targets_vae(self, tau=1.0):
    """Performs a soft update of the target network parameters.
    ...
    update_actor = utils.soft_variables_update(
        slim.get_trainable_variables(
            utils.join_scope(self._scope, self.ACTOR_NET_VAE_SCOPE)),
        slim.get_trainable_variables(
            utils.join_scope(self._scope, self.TARGET_ACTOR_NET_VAE_SCOPE)),
        tau)
    update_critic = utils.soft_variables_update(
        slim.get_trainable_variables(
            utils.join_scope(self._scope, self.CRITIC_NET_VAE_SCOPE)),
        slim.get_trainable_variables(
            utils.join_scope(self._scope, self.TARGET_CRITIC_NET_VAE_SCOPE)),
        tau)
    return tf.group(update_actor, update_critic, name='update_targets_vae')

  @gin.configurable('ddpg_update_targets_real_nvp')
  def update_targets_real_nvp(self, tau=1.0):
    """Performs a soft update of the target network parameters.
    ...
    update_actor = utils.soft_variables_update(
        slim.get_trainable_variables(
            utils.join_scope(self._scope, self.ACTOR_NET_REAL_NVP_SCOPE)),
        slim.get_trainable_variables(
            utils.join_scope(self._scope, self.TARGET_ACTOR_NET_REAL_NVP_SCOPE)),
        tau)
    update_critic = utils.soft_variables_update(
        slim.get_trainable_variables(
            utils.join_scope(self._scope, self.CRITIC_NET_REAL_NVP_SCOPE)),
        slim.get_trainable_variables(
            utils.join_scope(self._scope, self.TARGET_CRITIC_NET_REAL_NVP_SCOPE)),
        tau)
    return tf.group(update_actor, update_critic, name='update_targets_real_nvp')

  def get_trainable_critic_meta_vars(self):
  ...
  def get_trainable_critic_vars(self):
    ...
    return slim.get_trainable_variables(
        utils.join_scope(self._scope, self.CRITIC_NET_SCOPE))
  ###
  def get_trainable_critic_vae_vars(self):
    """Returns a list of trainable variables in the critic network.
  ...
  def get_trainable_critic_real_nvp_vars(self):
    """Returns a list of trainable variables in the critic network.
  ...
  def get_trainable_actor_meta_vars(self):
  ...
  def get_trainable_actor_vars(self):
    ...
    return slim.get_trainable_variables(
        utils.join_scope(self._scope, self.ACTOR_NET_SCOPE))
  ###
  def get_trainable_actor_vae_vars(self):
    """Returns a list of trainable variables in the actor network.
  ...
  def get_trainable_actor_real_nvp_vars(self):
    """Returns a list of trainable variables in the actor network.
  ...
  def get_actor_meta_vars(self):
    ...
    """
    return slim.get_model_variables(
        ##utils.join_scope(self._scope, self.ACTOR_NET_META_SCOPE))
        utils.join_scope(self._scope, self.ACTOR_NET_SCOPE))
  ###
  ...
  def get_actor_real_nvp_vars(self):
    """Returns a list of all variables in the actor network.
  ...
  def get_mlpfunction_vars(self):

    return self._observations_preprocessor.get_params_internal()
  ###
  ...
  ###
  def _validate_original_states_contexts(self, original_states, contexts):
    """Raises a value error if `states` does not have the expected shape.
    ...
    """
    if self.mode != 'uvf':
        original_states.shape.assert_is_compatible_with(self._original_observation_spec_shape)
    if not original_states.dtype.is_compatible_with(self._original_observation.dtype):
      raise ValueError('states.dtype={} is not compatible with'
                       ' observation_spec.dtype={}'.format(
          original_states.dtype, self._original_observation.dtype))
  ###
  ...
  ###
  def inverse_low_actions(self, low_actions, states):

    low_actions_list = []

    for x in range(low_actions.get_shape().as_list()[0]):
        low_actions_list.append(low_actions[x][0])

    low_actions_stack = tf.stack(low_actions_list)

    states_list = []

    for x in range(states.get_shape().as_list()[0]):
        states_list.append(states[x][0])

    states_stack = tf.stack(states_list)

    context = self._bijector.inverse(low_actions_stack, condition=states_stack)
...
class TD3Agent(DdpgAgent):
  ...
  ACTOR_NET_META_SCOPE = 'actor_net_meta'
  CRITIC_NET_META_SCOPE = 'critic_net_meta'
  CRITIC_NET2_META_SCOPE = 'critic_net2_meta'
  TARGET_ACTOR_NET_META_SCOPE = 'target_actor_net_meta'
  TARGET_CRITIC_NET_META_SCOPE = 'target_critic_net_meta'
  TARGET_CRITIC_NET2_META_SCOPE = 'target_critic_net2_meta'
  ###
  ACTOR_NET_SCOPE = 'actor_net'
  CRITIC_NET_SCOPE = 'critic_net'
  ...
  ACTOR_NET_VAE_SCOPE = 'actor_net_vae'
  CRITIC_NET_VAE_SCOPE = 'critic_net_vae'
  CRITIC_NET2_VAE_SCOPE = 'critic_net2_vae'
  TARGET_ACTOR_NET_VAE_SCOPE = 'target_actor_net_vae'
  TARGET_CRITIC_NET_VAE_SCOPE = 'target_critic_net_vae'
  TARGET_CRITIC_NET2_VAE_SCOPE = 'target_critic_net2_vae'

  ###real_nvp
  ACTOR_NET_REAL_NVP_SCOPE = 'actor_net_real_nvp'
  CRITIC_NET_REAL_NVP_SCOPE = 'critic_net_real_nvp'
  CRITIC_NET2_REAL_NVP_SCOPE = 'critic_net2_real_nvp'
  TARGET_ACTOR_NET_REAL_NVP_SCOPE = 'target_actor_net_real_nvp'
  TARGET_CRITIC_NET_REAL_NVP_SCOPE = 'target_critic_net_real_nvp'
  TARGET_CRITIC_NET2_REAL_NVP_SCOPE = 'target_critic_net2_real_nvp'
  ...
  def __init__(self,
    ...
    ###
    self._original_observation = original_observation_spec
    self._original_context = original_context_specs

    self._original_observation_spec = original_observation_spec.shape
    self._original_context_specs = original_context_specs[0].shape

    self._original_observation_spec_shape = tf.TensorShape([None]).concatenate(
         self._original_observation_spec)
    self._original_context_specs_shape = tf.TensorShape([None]).concatenate(
         self._original_context_specs)
    ###
    self._scope = tf.get_variable_scope().name
	###
    if self.mode == 'uvf':

        policy_params = LSP_POLICY_PARAMS_BASE
        ...
        preprocessing_hidden_sizes = BIJECTOR_PARAMS.get('preprocessing_hidden_sizes')
        outputndims = BIJECTOR_PARAMS.get('outputndims')

        with tf.variable_scope('main_observation'):
            if preprocessing_hidden_sizes is not None:
                observations_preprocessor = MLPPreprocessor(
                    ...
                    outputndims = outputndims)
            else:
                observations_preprocessor = None

            self._observations_preprocessor = observations_preprocessor

        with tf.variable_scope('target_observation'):
            if preprocessing_hidden_sizes is not None:
                target_observations_preprocessor = MLPPreprocessor(
                    layer_sizes = preprocessing_hidden_sizes,
                    output_nonlinearity = nonlinearity,
                ...
                target_observations_preprocessor = None

            self._target_observations_preprocessor = target_observations_preprocessor

        policy_s_t_layers = BIJECTOR_PARAMS.get("policy_s_t_layers")
        policy_s_t_units = BIJECTOR_PARAMS.get("policy_s_t_units")
        s_t_hidden_sizes = [policy_s_t_units] * policy_s_t_layers

        self._reparameterize = LSP_POLICY_PARAMS_BASE.get("reparameterize")
        self._squash = LSP_POLICY_PARAMS_BASE.get("squash")

        bijector_config = {
            "scale_regularization": 0.0,
            "num_coupling_layers": BIJECTOR_PARAMS.get("policy_coupling_layers"),
        ...
        with tf.variable_scope('main_observation'):
            self._bijector = RealNVPBijector(
                num_coupling_layers=bijector_config.get("num_coupling_layers"),
                translation_hidden_sizes=bijector_config.get("translation_hidden_sizes"),
                scale_hidden_sizes=bijector_config.get("scale_hidden_sizes"),
                event_ndims=int(self._num_action_dims),
                name="bijector_real_nvp")

        with tf.variable_scope('target_observation'):
            self._target_bijector = RealNVPBijector(
                num_coupling_layers=bijector_config.get("num_coupling_layers"),
                translation_hidden_sizes=bijector_config.get("translation_hidden_sizes"),
                scale_hidden_sizes=bijector_config.get("scale_hidden_sizes"),
                event_ndims=int(self._num_action_dims),
                name="target_real_nvp")

        ds = tf.contrib.distributions

        self.base_distribution = ds.MultivariateNormalDiag(
            loc=tf.zeros(int(self._num_action_dims)), scale_diag=tf.ones(int(self._num_action_dims)))

        self.sample_z = self.base_distribution.sample(1)

        self._distribution = ds.ConditionalTransformedDistribution(
	...
	###
        ...
        self._critic_net_vae = tf.make_template(
            self.CRITIC_NET_SCOPE, critic_net_vae, create_scope_now_=True)
        self._critic_net2_vae = tf.make_template(
            self.CRITIC_NET2_SCOPE, critic_net_vae, create_scope_now_=True)
        self._target_actor_net_vae = tf.make_template(
            self.TARGET_ACTOR_NET_SCOPE, actor_net_vae, create_scope_now_=True)
        self._target_critic_net_vae = tf.make_template(
            self.TARGET_CRITIC_NET_SCOPE, critic_net_vae, create_scope_now_=True)
        self._target_critic_net2_vae = tf.make_template(
            self.TARGET_CRITIC_NET2_SCOPE, critic_net_vae, create_scope_now_=True)

        self.action_dist_logstd_param = tf.Variable(
            (.01 * np.random.randn(1, self._action_spec.shape.num_elements())).astype(np.float32),
            name="policy_logstd")

        self.oldaction_dist_mu = tf.get_variable('oldaction_dist_mu', dtype=tf.float32, shape=[100, self._num_action_dims], initializer=tf.zeros_initializer())
        self.oldaction_dist_logstd = tf.get_variable('oldaction_dist_logstd', dtype=tf.float32, shape=[100, self._num_action_dims], initializer=tf.zeros_initializer())

        self.old_means = tf.get_variable('old_means', dtype=tf.float32, shape=[100, self._num_action_dims], initializer=tf.zeros_initializer())
        self.old_log_vars = tf.get_variable('old_log_vars', dtype=tf.float32, shape=[100, self._num_action_dims], initializer=tf.zeros_initializer())
        ...
        self._critic_net_real_nvp = tf.make_template(
            self.CRITIC_NET_SCOPE, critic_net_real_nvp, create_scope_now_=True)
        self._critic_net2_real_nvp = tf.make_template(
            self.CRITIC_NET2_SCOPE, critic_net_real_nvp, create_scope_now_=True)
        self._target_actor_net_real_nvp = tf.make_template(
            self.TARGET_ACTOR_NET_SCOPE, target_actor_net_real_nvp, create_scope_now_=True)
        self._target_critic_net_real_nvp = tf.make_template(
            self.TARGET_CRITIC_NET_SCOPE, critic_net_real_nvp, create_scope_now_=True)
        self._target_critic_net2_real_nvp = tf.make_template(
            self.TARGET_CRITIC_NET2_SCOPE, critic_net_real_nvp, create_scope_now_=True)
  ...
  def critic_net_vae(self, states, actions, for_critic_loss=False):
    ...
    Raises:
      ValueError: If `states` or `actions' do not have the expected dimensions.
    """
    values1 = self._critic_net_vae(states, actions,
                               for_critic_loss=for_critic_loss)
    values2 = self._critic_net2_vae(states, actions,
                                for_critic_loss=for_critic_loss)
    if for_critic_loss:
      return values1, values2
    return values1
  ...
  def critic_net_real_nvp(self, states, actions, for_critic_loss=False):
    ...
    Raises:
      ValueError: If `states` or `actions' do not have the expected dimensions.
    """
    values1 = self._critic_net_real_nvp(states, actions,
                               for_critic_loss=for_critic_loss)
    values2 = self._critic_net2_real_nvp(states, actions,
                                for_critic_loss=for_critic_loss)
    if for_critic_loss:
      return values1, values2
    return values1
  ...
  def target_critic_net_vae(self, states, actions, for_critic_loss=False):
    ...
    values1 = tf.stop_gradient(
        self._target_critic_net_vae(states, actions,
                                for_critic_loss=for_critic_loss))
    values2 = tf.stop_gradient(
        self._target_critic_net2_vae(states, actions,
                                 for_critic_loss=for_critic_loss))
    if for_critic_loss:
      return values1, values2
    return values1
  ...
  def target_critic_net_real_nvp(self, states, actions, for_critic_loss=False):
    ...
    values1 = tf.stop_gradient(
        self._target_critic_net_real_nvp(states, actions,
                                for_critic_loss=for_critic_loss))
    values2 = tf.stop_gradient(
        self._target_critic_net2_real_nvp(states, actions,
                                 for_critic_loss=for_critic_loss))
    if for_critic_loss:
      return values1, values2
    return values1
  ...
  def target_value_net_vae(self, states, for_critic_loss=False):
    ...
    noise = tf.clip_by_value(
        tf.random_normal(tf.shape(target_actions), stddev=0.2), -0.5, 0.5)
    values1, values2 = self.target_critic_net_vae(
        states, target_actions + noise,
        for_critic_loss=for_critic_loss)
    values = tf.minimum(values1, values2)
    return values, values
  ...
  def target_value_net_real_nvp(self, states, original_states, contexts, for_critic_loss=False):
    ...
    noise = tf.clip_by_value(
        tf.random_normal(tf.shape(target_actions), stddev=0.2), -0.5, 0.5)
    values1, values2 = self.target_critic_net_real_nvp(
        states, target_actions + noise,
        #original_states, target_actions + noise,
        for_critic_loss=for_critic_loss)
    values = tf.minimum(values1, values2)
    return values, values

  @gin.configurable('td3_update_targets_meta')
  def update_targets_meta(self, tau=1.0):
    """Performs a soft update of the target network parameters.
  ...
  def update_targets(self, tau=1.0):
    ...
    return tf.group(update_actor, update_critic, name='update_targets')
  ###
  @gin.configurable('td3_update_targets_vae')
  def update_targets_vae(self, tau=1.0):
    """Performs a soft update of the target network parameters.
  ...
  @gin.configurable('td3_update_targets_real_nvp')
  def update_targets_real_nvp(self, tau=1.0):
    """Performs a soft update of the target network parameters.
