...
class UvfAgentCore(object):
  ...
  def __init__(self,
	...
	###
    self._num_state_dims = self.env_observation_spec.shape.as_list()[0]
	###
    ...
    self.BASE_AGENT_CLASS.__init__(
		...
		###
        original_observation_spec = self.env_observation_spec,
        original_context_specs = self.context_specs,
		###
        action_spec=action_spec,
		###
        num_state_dims = self._num_state_dims,
		###
        **base_agent_kwargs
    )
  ...
  ###
  def actor_loss_vae(self, states, actions, rewards, discounts,
                 next_states):
    """Returns the next action for the state.
    ...
    Returns:
      A [num_action_dims] tensor representing the action.
    """
    return self.BASE_AGENT_CLASS.actor_loss_vae(self, states)
  ...
  def actor_loss_real_nvp(self, states, original_states, contexts):
    ...
    Returns:
      A [num_action_dims] tensor representing the action.
    """
    return self.BASE_AGENT_CLASS.actor_loss_real_nvp(self, states, original_states, contexts)

  def action_vaes(self, merged_state):
      action_vaes = self.BASE_AGENT_CLASS.action_vaes(self, merged_state)
      return action_vaes

  def action_z_state(self, merged_state):
      action_z_state = self.BASE_AGENT_CLASS.action(self, merged_state)
      return  action_z_state

  def action_z_states(self, merged_state):
      action_z_states = self.BASE_AGENT_CLASS.actions(self, merged_state)
      return action_z_states

  def action_real_nvp(self, state, context):
      action_real_nvp = self.BASE_AGENT_CLASS.action_real_nvp(self, state, context)
      return  action_real_nvp

  def action_real_nvps(self, state, context):
      action_real_nvps = self.BASE_AGENT_CLASS.action_real_nvps(self, state, context)
      return  action_real_nvps
  ###
  def action(self, state, context=None):
	...
	###
    action_z = self.BASE_AGENT_CLASS.action_vae(self, merged_state)
    merged_state = self.merged_state_with_context(state, action_z)
    action_z_state = self.BASE_AGENT_CLASS.action(self, merged_state)
    state = action_z_state
    action_real_nvp = self.BASE_AGENT_CLASS.action_real_nvp(self, state, context)
    return action_z, action_z_state, action_real_nvp
	###
  def actions(self, state, context=None):
    ...
    #### be careful for this order : action_z -> action -> action_real_nvp
    merged_state = self.merged_states(state, context)
	###
    action_z = self.BASE_AGENT_CLASS.action_vaes(self, merged_state)
    action_z_state = self.BASE_AGENT_CLASS.actions(self, merged_state)
    state = action_z_state
    action_real_nvp = self.BASE_AGENT_CLASS.action_real_nvps(self, state, context)
    return action_z, action_z_state, action_real_nvp
	###
  ###
  def meta_action(self, state, context=None):
    """Returns the next action for the state.
  ...
  def meta_actions(self, state, context=None):
    """Returns the next action for the state.
  ...
  def log_probs(self, states, actions, state_reprs, contexts=None):
	...
	###
    action_z, action_z_state, action_real_nvp = self.actions(flat_states, flat_contexts)
    flat_pred_actions = action_z
	###
  ...
  def add_noise_fn(self, action_fn, stddev=1.0, debug=False,
    ...
    def noisy_action_fn(state, context=None):
      ...
      if self.mode == 'meta':
        ...
        action_z, action_z_state, action_real_nvp = action_fn(state, context)
        action = action_z
	  ###
      if debug:
	  ...
	  ###
        ...
        return action, action_z_state, action_real_nvp
	  ###
  ...
  def merged_state(self, state, context=None):
    ...
    return state
  ###
  def merged_state_with_context(self, state, context=None):
    """Returns the merged state from the environment state and contexts.
    ...
    '''
    if context is None:
      print("merged_state context is none")
      context = list(self.context_vars)
    '''
    state = tf.concat([state,] + [context,], axis=-1)
    self._validate_states(self._batch_state(state))
  ...
  ###
  def inverse_real_nvp(self, low_actions, states):
    return self.BASE_AGENT_CLASS.inverse_low_actions(self, low_actions, states)
  ###
